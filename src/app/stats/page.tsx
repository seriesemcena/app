        ...
        <ScrollArea style={{ padding: '0 0 32px' }}>
          ...
          <div style={{ display:'flex', gap:8, margin:'12px 16px 0' }}>
            {([{ id:'series', label: t('statsPage.series') },{ id:'filmes', label: t('statsPage.moviesTab') }] as const).map(({ id, label }) => (
              <button key={id} onClick={() => setTab(id)} style={{ padding:'8px 20px', borderRadius:24, background: tab===id ? T.pillActiveBg : 'transparent', border: tab===id ? `1px solid ${T.pillActiveBorder}` : `1px solid ${T.border}`, color: tab===id ? T.pillActiveText : T.t2, fontSize:14, fontWeight: tab===id ? 800 : 600, fontFamily:"'Area','Inter',sans-serif", cursor:'pointer', transition:'all 0.2s' } as React.CSSProperties}>
                {label}
              </button>
            ))}
          </div>
