...
const STREAM_LOGOS: Record<string, string> = {
  netflix: 'netflix',
  prime: 'primevideo',
  disney: 'dineyplus',
  hbo: 'hbomax',
...
const PRESET_STREAMS: Stream[] = [
  { id: 'netflix', name: 'Netflix', color: '#E50914', plans: [
    { label: 'Padrão com anúncios', price: 20.90 },
  ...
  { id: 'hbo', name: 'HBO Max', color: '#5800A0', plans: [
    ...
    { label: 'Básico anual (R$ 226,80/ano)', price: 226.80 / 12 },
    { label: 'Standard anual (R$ 358,80/ano)', price: 358.80 / 12 },
  ...
  { id: 'globo', name: 'Globoplay', color: '#D62929', plans: [
    ...
    { label: 'Premium anual', price: 29.90 },
  ] },
  { id: 'paramount', name: 'Paramount+', color: '#0064FF', plans: [
    { label: 'Básico', price: 27.90 },
    { label: 'Padrão', price: 34.90 },
    { label: 'Premium', price: 44.90 },
    { label: 'Básico anual (R$ 249,90/ano)', price: 249.90 / 12 },
...
function StreamingLogo({ streamId, name, isDark, size }: { streamId: string; name: string; isDark: boolean; size: number }) {
  const logo = STREAM_LOGOS[streamId];
  return (
    <div style={{
      ...
      {logo ? (
        ...
        <img
          src={`/${logo}_logo${isDark ? '' : '_black'}.png`}
          alt=""
          style={{ width: '70%', height: '62%', objectFit: 'contain', display: 'block' }}
        />
      ) : (
        <Txt size={13} weight={800} color={T.t2}>{name[0]}</Txt>
      )}
    </div>
  );
}
  ...
  return (
    <Frame>
      <Screen>
        <ScrollArea>
          <GlassHeader
            navTitle={t('expenses.title')}
            showNavTitle={showNavTitle}
            left={
              <button onClick={() => navigateBack(router)} style={{ width: 34, height: 34, borderRadius: 17, background: isDark ? 'rgba(255,255,255,0.10)' : 'rgba(0,0,0,0.08)', border: isDark ? '1px solid rgba(255,255,255,0.15)' : '1px solid rgba(0,0,0,0.12)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 4 }}>
              ...
              <Txt size={14} weight={400} color="rgba(255,255,255,0.32)">{t('expenses.perMonth')}</Txt>
            </div>

            {/* Annual + mini stats */}
            <Txt size={12} color="rgba(255,255,255,0.38)" style={{ display: 'block', marginBottom: 16 }}>
              R$ {formatPrice(annual)} {t('expenses.perYear')} · {t('expenses.active', { count: activeSubs.length })}
              {subs.length - activeSubs.length > 0 && `, ${t('expenses.paused', { count: subs.length - activeSubs.length })}`}
            </Txt>
          ...
          {subs.length > 0 && (
            <div style={{ margin: '16px 16px 0', padding: 16, background: T.card, borderRadius: T.radiusSm, border: `1px solid ${T.border}` }}>
              <Txt size={11} color={T.t3} weight={700} style={{ display: 'block', marginBottom: 12, textTransform: 'uppercase', letterSpacing: 1 }}>{t('expenses.distribution')}</Txt>
              {subs.map((s) => (
                <div key={s.id} style={{ marginBottom: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      {!s.active && <div style={{ padding: '1px 6px', borderRadius: 4, background: T.surface2 }}><Txt size={9} color={T.t3}>{t('expenses.pausedTag')}</Txt></div>}
                    </div>
                    <Txt size={12} color={s.active ? T.t1 : T.t3}>R$ {formatPrice(s.price)}</Txt>
                  </div>
          ...
          <div style={{ padding: '16px 16px 0' }}>
            <div style={{ marginBottom: 12 }}>
              <Txt size={16} weight={700}>{t('expenses.mySubscriptions')}</Txt>
            </div>

            {subs.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px 24px' }}>
                <Icon name="play" size={48} color={T.t4} style={{ marginBottom: 16 }} />
                <Txt size={15} weight={700} style={{ display: 'block', marginBottom: 8 }}>{t('expenses.emptyTitle')}</Txt>
                <Txt size={13} color={T.t3} style={{ display: 'block' }}>{t('expenses.emptyDetail')}</Txt>
              </div>
            ) : (
              <>
                {subs.map((s) => (
                  <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', background: T.card, borderRadius: T.radiusSm, border: `1px solid ${T.border}`, marginBottom: 10, opacity: s.active ? 1 : 0.6 }}>
                    <StreamingLogo streamId={s.streamId} name={s.name} isDark={isDark} size={44} />
                    <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', marginTop: 16 }}>
                  ...
                  >
                    {t('expenses.addAnotherStreaming')}
                  </button>
                </div>
              </>
            )}
        ...
        <BottomSheet visible={addSheet} onClose={() => { setAddSheet(false); setSelStream(null); }} title={t('expenses.addSheetTitle')}>
          {!selStream ? (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {PRESET_STREAMS.map((s) => (
                <button key={s.id} onClick={() => setSelStream(s)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 12, background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.radiusSm, cursor: 'pointer' }}>
                  <StreamingLogo streamId={s.id} name={s.name} isDark={isDark} size={36} />
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, padding: 12, background: T.surface, borderRadius: T.radiusSm }}>
                <StreamingLogo streamId={selStream.id} name={selStream.name} isDark={isDark} size={40} />
                <Txt size={15} weight={700}>{selStream.name}</Txt>
              </div>
              <Txt size={12} color={T.t3} weight={700} style={{ display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>{t('expenses.plan')}</Txt>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
                {selStream.plans.map((p, i) => (
                  <button key={i} onClick={() => setSelPlan(i)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: selPlan === i ? 'rgba(192,105,255,0.12)' : T.surface, border: `1px solid ${selPlan === i ? T.pink : T.border}`, borderRadius: T.radiusSm, cursor: 'pointer' }}>
                    ...
                    <Txt size={13} weight={700} color={selPlan === i ? T.pink : T.t3}>R$ {formatPrice(p.price)}{t('expenses.perMonth')}</Txt>
                  </button>
                ))}
              </div>
              <Txt size={12} color={T.t3} weight={700} style={{ display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>{t('expenses.customPrice')}</Txt>
              <input value={customPrice} onChange={(e) => setCustomPrice(e.target.value)} placeholder={t('expenses.customPricePlaceholder')}
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <Btn label={t('expenses.back')} variant="ghost" onClick={() => setSelStream(null)} style={{ flex: 1 }} />
                <Btn label={t('expenses.add')} variant="pink" onClick={addSub} style={{ flex: 2 }} />
              </div>
            </div>
          )}
        </BottomSheet>
